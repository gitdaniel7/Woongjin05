...
const SideBar = ({ footerRef }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // 사이드바가 푸터와 겹칠 수 있는 최대 위치 계산
      const sideBarBottomPosition =
        windowHeight - sideBarBottomFixed + sideBarHeight;

      if (sideBarBottomPosition > footerTop) {
        // 푸터와 겹치면 absolute로 변경하고 푸터 바로 위에 고정
        setIsFixed(false);
        // 푸터의 절대 위치를 계산하여 사이드바 위치 설정
        const footerAbsoluteTop = footerRect.top + window.scrollY;
        setTop(`${footerAbsoluteTop - sideBarHeight - 10}px`);
        setBottom("auto");
      } else {
      ...
      <button
        onClick={() =>
          (window.location.href =
            "https://www.mcdonalds.co.kr/kor/story/competition/farmToRestaurant.do")
        }
        className="w-[121px] h-[121px] mb-5 bg-no-repeat bg-center cursor-pointer border-none bg-transparent"
        style={{
          backgroundImage:
            "url('https://www.mcdonalds.co.kr/kor/images/common/btn_story_04.png')",
        }}
        aria-label="사이드바 버튼"
      />
      <button
        ...
        style={{
          backgroundImage:
            "url('https://www.mcdonalds.co.kr/kor/images/common/btn_top_04.png')",
        }}
        aria-label="스크롤 탑 버튼"
      />
    </div>
